...
async def get_appointements(user: User = role_required(Role.ADMIN, Role.Super_Admin), skip: int = 0, limit: int = 10):
    return  await appointemntService.get_all_appointement( skip, limit)
...
async def create_appointement(appontCreate : AppointmentCreate, user: User = role_required( Role.ADMIN, Role.Super_Admin)):
    ...
    appointement: Appointment =  await appointemntService.create_appointement(admin=user, student=student, order=order, scheduled_at=appontCreate.scheduled_at, location=appontCreate.location)
    
    id = str(appointement.id)
    return {"apppoitement_id": id}
